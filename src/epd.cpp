...
void EPD::init() {
    ...
    delay(10);          // SSD1681 requires ≥10ms after SWRESET; waitBusy() alone is unreliable here
    waitBusy();
...
void EPD::display(const uint8_t* black_buf, const uint8_t* red_buf) {
    ...
    delay(16000);       // GPIO36 (BUSY) has no pull-up and floats LOW when idle, so waitBusy() exits
                        // immediately. Hard-wait 16s to cover the full ~15s 3-color refresh waveform.
    waitBusy();
}
